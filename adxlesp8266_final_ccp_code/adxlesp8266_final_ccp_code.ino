...
/*
#define BLYNK_TEMPLATE_NAME "Fall Detection and Prevention"
#define BLYNK_AUTH_TOKEN "xEBRILHzIFum6ClnDJGxCl--wm4JlcP_"
...
float gettiltAngle(float ax, float ay, float az) {
    ...
    return acos(az / magnitude) * 180.0 / PI;
}

/*void setupFreeFall() {
    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x28); Wire.write(5); // Threshold: 312.5 mg
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x29); Wire.write(10); // Time: 50 ms
    Wire.endTransmission();

    /*Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x2E); Wire.write(0x04); // Enable FREE_FALL interrupt
    Wire.endTransmission();
}

void setupActivityInactivity() {
    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x28); Wire.write(5); // Threshold: 312.5 mg free fall
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x29); Wire.write(10); // Time: 50 ms free fall
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x24); Wire.write(10); // Activity threshold: 600 mg
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x25); Wire.write(5); // Inactivity threshold: 300 mg
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x26); Wire.write(5); // Inactivity time: 5 seconds
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x27); Wire.write(0xFF); // Enable on all axes
    Wire.endTransmission();

    Wire.beginTransmission(ADXL345_ADDR);
    Wire.write(0x2E); Wire.write(0x1C); // Enable ACTIVITY and INACTIVITY interrupts
    Wire.endTransmission();
}
...
void loop() {
    ...
    if (isActive) {
        Serial.print("Ax: "); Serial.print(event.acceleration.x);Blynk.virtualWrite(event.acceleration.x);
        Serial.print(" | Ay: "); Serial.print(event.acceleration.y);Blynk.virtualWrite(event.acceleration.y);
        Serial.print(" | Az: "); Serial.println(event.acceleration.z);Blynk.virtualWrite(event.acceleration.z);
    }
...
#define BLYNK_TEMPLATE_NAME "Fall Detection and Prevention"
#define BLYNK_AUTH_TOKEN "xEBRILHzIFum6ClnDJGxCl--wm4JlcP_"
#define BLYNK_TEMPLATE_ID "TMPL3ol5U3j8w"
#include <Wire.h>
#include <Adafruit_Sensor.h>
#include <Adafruit_ADXL345_U.h>
#include <ESP8266WiFi.h>
